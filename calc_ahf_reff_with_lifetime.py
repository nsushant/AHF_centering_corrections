...
cen_file_AHF = pd.read_csv(AHF_cen_file)
...
for i in range(len(snapshots)):
    ...
    if (np.isin(i,cen_file_AHF['i']) == False):
        print('snap has no matching output')
        continue
    
    cross_reference_haloID = cen_file_AHF[cen_file_AHF['i'] == i]['AHF catalogue id'].values[0]
    ...
    children_ahf = cen_file_AHF[cen_file_AHF['i'] == i]['children'].values[0]
    ...
    sub_halonums = np.asarray(children_ahf_int)
    st_children = np.array([])                                                                                                                                                            
    dm_children = np.array([])

    for i in children_ahf_int:

        dm_children = np.append(dm_children,h_AHF[int(i)].dm['iord'])
        st_children = np.append(st_children,h_AHF[int(i)].st['iord'])
    ...
    if ((iords_exist == 1) and (DMO_state == 'fiducial')):
        if len(main_AHF_halo.st["iord"])>0:
            ...
            try:
                pynbody.analysis.halo.center(main_h_stars.st)

            except:
                print('unable to calculate_center')
...
#df_centers.to_csv('stored_centers.csv')

#if (DMO_state=='fiducial'):
    
df = pd.DataFrame({'reff':reffs,'i':irrs,'AHF catalogue id':crossreffids,'t':ts})
#,'reffs':reffs})
print('wrote',cen_storage_file)
